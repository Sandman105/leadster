...
const subscriptionSchema = knex.schema.hasTable("subscription").then(table => {
    if (!table) { //if table doesn't exist then create it
        return knex.schema.createTable("subscription", table => {
            ...
            table.integer("userID").unsigned().references("users.id").onDelete("CASCADE"); //unsigned means only positive numbers
            table.integer("postingID").unsigned().references("posting.id").onDelete("CASCADE");
